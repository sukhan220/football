...
export async function GET(req: Request) {
  try {
    ...
    // ২. COMPETITION আপসার্ট (apiCode দিয়ে ট্র্যাকিং)
    const dbCompetition = await prisma.competition.upsert({
      where: { slug: compSlug },
      update: { apiCode: competitionCode },
      create: {
    ...
    // 🌟 ডামি 'To Be Determined' টিম এনশিওর করা (নকআউট ম্যাচের জন্য)
    const tbdTeam = await prisma.team.upsert({
      ...
      create: {
        ...
        logo: null, // এখানে চাইলে কোনো ডিফল্ট প্লেসহোল্ডার ইমেজের লিংক দিতে পারেন
        isNational: competitionCode === "WC"
      }
    });

    // ৪. টিমগুলো ডাটাবেজে এনশিওর করা (apiId দিয়ে ট্র্যাকিং)
    const dbTeamsCache: Record<number, string> = {}; // দ্রুত আইডির রিলেশন খোঁজার জন্য মেমোরি ক্যাশ

    for (const match of matches) {
      // হোম টিম প্রসেস (যদি আইডি থাকে তবেই প্রসেস হবে)
      if (match.homeTeam?.id && !dbTeamsCache[match.homeTeam.id]) {
        const homeTeamSlug = `${slugify(match.homeTeam.name)}-${match.homeTeam.id}`; // ডুপ্লিকেট স্লগ এড়াতে আইডি যুক্ত
        const team = await prisma.team.upsert({
          where: { apiId: match.homeTeam.id },
          update: { logo: match.homeTeam.crest || null },
          create: {
    ...
    // ৫. MATCHES আপসার্ট (apiId দিয়ে ট্র্যাকিং)
    const syncedMatches = [];
    
    for (const match of matches) {
      const currentStatus = statusMapping[match.status] || "SCHEDULED";
      
      // 🌟 আইডি থাকলে ক্যাশ থেকে আনবে, না থাকলে TBD টিমের আইডি বসিয়ে দেবে
      ...
      const matchRecord = await prisma.match.upsert({
        ...
        update: {
          homeTeamId: homeTeamUUID, // 🌟 আপডেট ব্লকে টিম আইডি রাখতেই হবে। কারণ পরে TBD বদলে আসল টিম আসবে।
          awayTeamId: awayTeamUUID,
          homeScore: match.score?.fullTime?.home ?? 0,
          awayScore: match.score?.fullTime?.away ?? 0,
          status: currentStatus,
