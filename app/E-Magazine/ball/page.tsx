...
//   // ডেসক্রিপশন স্ক্রলযোগ্য করার স্টেট
//   const [isExpanded, setIsExpanded] = useState(false);
//   // ইমেজ বড় করে দেখার (Lightbox) স্টেট
//   const [previewImage, setPreviewImage] = useState<string | null>(null);

//   // নতুন বল সিলেক্ট হলে স্ক্রল স্টেট আগের জায়গায় ফিরে যাবে এবং প্যারালাক্স চালু হবে
//   useEffect(() => {
//     if (isExpanded) {
//       handleCloseExpanded();
//     }
...
//   // Expand বন্ধ করার ফাংশন (প্যারালাক্স আবার চালু করবে)
...
//   // Expand চালু করার ফাংশন (প্যারালাক্স বন্ধ করবে)
//   const handleOpenExpanded = () => {
//     setIsExpanded(true);
//     if (lenisRef.current) lenisRef.current.stop();
//     if (mainTriggerRef.current) mainTriggerRef.current.disable(false); // disable but keep current visual position
//   };
...
//     // মোবাইল ভিউতে বলগুলোর দূরত্ব বাড়ানোর জন্য ১৪০ থেকে বাড়িয়ে ২০০ করা হয়েছে
...
//     <div
//       ref={containerRef}
//       className="relative w-full h-screen overflow-hidden bg-[#0c111d] text-white font-outfit select-none z-10"
...
//               <div
//                 key={ball.id}
//                 onClick={() => setPreviewImage(ball.image)} // বল ক্লিক করলে ইমেজ বড় হবে
...
//       {/* ফুল স্ক্রিন স্ক্রলযোগ্য টেক্সট লেয়ার (মোর দিলে ব্যাকগ্রাউন্ড সম্পূর্ণ ট্রান্সপারেন্ট থাকবে) */}
//       {isExpanded && (
//         <div className="fixed inset-0 z-50 bg-transparent overflow-y-auto px-6 py-24 flex flex-col items-center">
//           {/* Close Button */}
//           <button 
//             onClick={handleCloseExpanded}
//             className="fixed top-6 right-6 text-gray-400 hover:text-white text-xl font-bold p-2 z-[60] bg-gray-900/80 rounded-full border border-white/10 cursor-pointer shadow-2xl"
//             title="Close"
//           >
//             ✕
//           </button>
...
//             {/* বিবরণী বক্সটি সম্পূর্ণ ট্রান্সপারেন্ট রাখা হয়েছে */}
//             <p className="text-sm md:text-base leading-relaxed text-white text-left bg-transparent p-5 rounded-xl drop-shadow-[0_4px_8px_rgba(0,0,0,1)] whitespace-pre-line mb-8 font-medium">
//               {fullDescription}
//             </p>

//             <button 
//               onClick={handleCloseExpanded}
//               className="px-6 py-2.5 bg-brand-500 text-black font-bold rounded-full text-xs uppercase tracking-wider hover:bg-brand-400 transition-colors shadow-2xl cursor-pointer"
//             >
//               {lang === "EN" ? "Close / Less" : "বন্ধ করুন"}
//             </button>
//           </div>
//         </div>
//       )}

//       {/* ইমেজ লাইটবক্স মডাল (বল ক্লিক করলে ইমেজ আলাদাভাবে বড় দেখাবে) */}
//       {previewImage && (
//         <div 
//           className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn"
//           onClick={() => setPreviewImage(null)} // যেকোনো জায়গায় ক্লিক করলে বন্ধ হবে
//         >
//           <button 
...
//         <span className="text-[8px] text-gray-500 tracking-wider block mb-0.5">INDEX</span>
...
export default function BallPerspectivePage() {
  ...
  // ডেসক্রিপশন স্ক্রলযোগ্য করার স্টেট
  const [isExpanded, setIsExpanded] = useState(false);
  // 이미지 বড় করে দেখার (Lightbox) স্টেট
  const [previewImage, setPreviewImage] = useState<string | null>(null);
  ...
  useGSAP(() => {
    ...
    // মোবাইল ভিউতে বলগুলোর দূরত্ব নিখুঁত রাখার স্টেপ
    const getStep = () => window.innerWidth < 768 ? 170 : 220;
    let step = getStep();
  ...
  return (
    <div
      ...
      style={{ contentVisibility: "auto" }}
    >
      {/* বাম পাশের নিচে কোণায় লোগো ওয়াটারমার্ক */}
