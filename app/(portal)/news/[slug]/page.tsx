...
import { Metadata } from "next"; // মেটাডাটা টাইপ ইমপোর্ট করা হলো

interface PageProps {
  params: Promise<{ slug: string }>;
}

// 🛠️ ফেসবুক প্রিভিউয়ের জন্য ডাইনামিক মেটাডাটা জেনারেটর ফাংশন
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;

  // স্লাগ অনুযায়ী ল্যাঙ্গুয়েজ ও আর্টিকেল ডাটা ফেচিং
  ...
  const { title, excerpt } = translationData;
  const { coverImage } = translationData.article;

  // ইমেজ যেন সবসময় absolute URL পায় সেটি নিশ্চিত করা (যেমন আপনার ডোমেন)
  const imageUrl = coverImage?.startsWith("http") 
    ? coverImage 
    : `https://dugoutadda.com${coverImage || "/default-news.png"}`;

  return {
    title: `${title} | Dugout Adda`,
    description: excerpt || "",
    openGraph: {
      title: title,
      description: excerpt || "",
      url: `https://dugoutadda.com/news/${slug}`,
      siteName: "Dugout Adda",
      type: "article",
      images: [
        {
          url: imageUrl,
...
export default async function SingleNewsPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#070b08] text-gray-100 py-20 selection:bg-emerald-500 selection:text-black">
